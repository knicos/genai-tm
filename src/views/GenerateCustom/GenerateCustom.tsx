...
import { FormControl, InputLabel, Select, MenuItem, SelectChangeEvent, Checkbox, FormControlLabel } from "@mui/material";
...
export default function GenerateCustom() {
    ...
    const doSubmit = useCallback((event: FormEvent<HTMLFormElement>) => {
        event.preventDefault();
        const urlCode = compressToEncodedURIComponent(JSON.stringify(delta(state)));
        navigate(`/image/general?c=${urlCode}`);
    }, [state, navigate]);

    const doCheckChange = useCallback((event: FormEvent<HTMLInputElement>) => {
        setState((old) => ({...old, [event.currentTarget.name]: event.currentTarget.checked}))
    }, [setState]);

    const doSelectChange = useCallback((event: SelectChangeEvent) => {
    ...
    return <div className={style.container}>
        ...
        <form onSubmit={doSubmit}>
            <FormControl fullWidth>
                ...
                <Select labelId="template-select" onChange={doSelectChange} value={state.namespace} label="Language Level" name="namespace">
                    <MenuItem value="image_adv">Advanced</MenuItem>
                    <MenuItem value="image_4_9">Grade 4-9</MenuItem>
                </Select>
            </FormControl>
            <FormControlLabel
                control={<Checkbox checked={state.advancedMenu} name="advancedMenu" onChange={doCheckChange} />}
                label="Show Advanced Training Menu"
            />
            <FormControlLabel
                control={<Checkbox checked={state.modelSelect} name="modelSelect" onChange={doCheckChange} />}
                label="Allow model selection"
            />
            <FormControlLabel
                control={<Checkbox checked={state.modelThreshold} name="modelThreshold" onChange={doCheckChange} />}
            ...
            />
            <FormControlLabel
                control={<Checkbox checked={state.disableAddClass} name="disableAddClass" onChange={doCheckChange} />}
                label="Disable adding classes"
            />
            <FormControlLabel
                control={<Checkbox checked={state.disabledClassRemove} name="disabledClassRemove" onChange={doCheckChange} />}
                label="Disable class removal"
            />
            <FormControlLabel
                control={<Checkbox checked={state.disableClassNameEdit} name="disableClassNameEdit" onChange={doCheckChange} />}
                label="Disable class name editing"
            />
            <FormControlLabel
                control={<Checkbox checked={state.imageBehaviours} name="imageBehaviours" onChange={doCheckChange} />}
                label="Image behaviours"
            />
            <FormControlLabel
                control={<Checkbox checked={state.soundBehaviours} name="soundBehaviours" onChange={doCheckChange} />}
                label="Sound behaviours"
            />
            <FormControlLabel
                control={<Checkbox checked={state.speechBehaviours} name="speechBehaviours" onChange={doCheckChange} />}
