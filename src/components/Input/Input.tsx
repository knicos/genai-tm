...
export default function Input({enabled, model, ...props}: Props) {
    ...
    const changeWebcamToggle = useCallback((event: React.ChangeEvent<HTMLInputElement>, checked: boolean) => {
    ...
    return <Widget dataWidget="input" title={t<string>("input.labels.title")} {...props}>
        <div className={style.container}>
            ...
            <div className={style.inputContainer}>
                {enabled && <Webcam disable={!enableInput} capture={enableInput && enabled} interval={200} onCapture={doPrediction}/>}
