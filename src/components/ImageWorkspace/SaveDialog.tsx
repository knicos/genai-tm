...
export default function SaveDialog({trigger, onSave, hasModel}: Props) {
    ...
    const changeSamples = useCallback((event: FormEvent<HTMLInputElement>) => {
        setSaveSamples(event.currentTarget.checked)
    }, [setSaveSamples]);

    const changeBehaviours = useCallback((event: FormEvent<HTMLInputElement>) => {
    ...
    return <Dialog open={!!trigger} onClose={trigger}>
        <DialogTitle>{t("save.title")}</DialogTitle>
        <DialogContent>
            <p>{(hasModel) ? t("save.message") : t("save.noModel")}</p>
            {!disableSaveSamples && <FormControlLabel
                control={<Checkbox checked={saveSamples} onChange={changeSamples} />}
            ...
            />
            <FormControlLabel
                control={<Checkbox disabled={!hasModel} checked={saveBehaviours} onChange={changeBehaviours} />}
                label={t("save.saveBehaviours")}
            />
        </DialogContent>
        <DialogActions>
            <Button variant="contained" disabled={!hasModel} onClick={doSave}>{t("save.actions.save")}</Button>
            <Button variant="outlined" onClick={trigger}>{t("save.actions.cancel")}</Button>
        </DialogActions>
    </Dialog>
}